...
#     for game in games:
#         genre = game['genre']
#         # ← O(n) imbriqué dans la boucle O(n) extérieure → O(n²)
#         genre_games = [g for g in games if g['genre'] == genre]
#         prices      = [g['price']  for g in genre_games]
...
def _calculate_stats_optimized(db):
  rows = db.execute("""
  SELECT
  genre,
  COUNT(*) AS count,
...
def games_stats():
    ...
    ⚠ Contient un bottleneck O(n²) dans _calculate_stats().
    → Endpoint cible du TP4 cProfile.
    """
    db = get_db()

    # Appel de la fonction lente
